...
print('Mixture:'.format(gas.X))
print('Cell size1 predictions ')
...
print('T (K), initial = {:1.5g}, 3,27final = {:1.5g}, max = {:1.5g} K'.format(out['T'][0],out['T'][-1],max(out['T'])))
print('P (Pa), initial = {:1.5g}, final = {:1.5g}, max = {:1.5g} Pa'.format(out['P'][0],out['P'][-1],max(out['P'])))
...
"""
Created on Tue Jun 11 21:19:07 2019

@author: JFk
"""
import matplotlib.pyplot as plt

t=[0,10,20,25,30,35,40,45,50,60,62.5,65]


#Cell size

gav=[1.686, 2.1, 2.768, 3.085, 3.625, 4.599, 5.934, 7.873, 12.02, 35.5, 53.36, 86.58]
ng=[1.986, 2.284, 2.637, 2.929, 3.223, 3.607, 4.16, 4.848, 5.741, 9.466, 11.11, 13.45]
westbrook=[0.8494, 0.9736, 1.136, 1.254, 1.393, 1.575, 1.821, 2.158, 2.617, 4.597, 5.584, 7.046]


plt.clf()
plt.plot(t, gav, label='Gav')
plt.plot(t, ng, label='Ng')
plt.plot(t, westbrook, label='Westbrook')
plt.axvline(x=67.5, ls='--', label='Detonation limit')
plt.legend(loc=0)
plt.ylabel('$Cell\ size$ [mm]')
plt.xlabel('$Volume\ fraction\ of\ N2\ in\ the\ mixture $ [%]')
plt.savefig('Cell size all.png')
plt.show()
...
plt.ylabel('$Cell\ size$ [mm]')
plt.xlabel('$Volume\ fraction\ of\ N2\ in\ the\ mixture $ [%]')
plt.savefig('Cell size gav.png')
plt.show()


plt.clf()
plt.plot(t, ng, label='Ng', color='orange')
plt.axvline(x=67.5, ls='--', label='Detonation limit')
plt.legend(loc=0)
plt.ylabel('$Cell\ size$ [mm]')
plt.xlabel('$Volume\ fraction\ of\ N2\ in\ the\ mixture $ [%]')
plt.savefig('Cell size Ng.png')
plt.show()


plt.clf()
plt.plot(t, westbrook, label='Westbrook', color='green')
plt.axvline(x=67.5, ls='--', label='Detonation limit')
plt.legend(loc=0)
plt.ylabel('$Cell\ size$ [mm]')
plt.xlabel('$Volume\ fraction\ of\ N2\ in\ the\ mixture $ [%]')
plt.savefig('Cell size wb.png')
plt.show()

#Temperature
T=[3683, 3586, 3492, 3440, 3383, 3321, 3251, 3171, 3077, 2832, 2753, 2664]

plt.clf()
plt.plot(t, T, label='Max temperature', color='orange')
plt.axvline(x=67.5, ls='--', label='Detonation limit')
plt.legend(loc=0)
...
#Pressure
P=[3.27, 3.21, 3.15, 3.11, 3.07, 3.02, 2.97, 2.91, 2.84, 2.64, 2.57, 2.5]

plt.clf()
plt.plot(t, P, label='Max pressure', color='orange')
plt.axvline(x=67.5, ls='--', label='Detonation limit')
plt.legend(loc=0)
plt.ylabel('$Pressure$ [MPa]')
plt.xlabel('$Volume\ fraction\ of\ N2\ in\ the\ mixture $ [%]')
plt.savefig('MaxPress.png')
plt.show()

#dT/dt max
dT_dis=[0.02929, 3.36E-02, 0.03916, 4.33E-02, 0.04813, 0.05449, 0.06236, 0.07395, 0.08971, 0.1563, 0.1902, 0.6946]
dT_time=[57.62, 70.8, 87.87, 99.92, 114.4, 133.32, 156.6, 190.8, 237.8, 438.5, 541.7, 694.6]


plt.clf()
plt.plot(t, dT_time, label='Time to (dT/dt)_max', color='orange')
plt.axvline(x=67.5, ls='--', label='Detonation limit')
plt.legend(loc=0)
plt.ylabel('$Time$ [ns]')
plt.xlabel('$Volume\ fraction\ of\ N2\ in\ the\ mixture $ [%]')
plt.savefig('Time to dTdt.png')
plt.show()

plt.clf()
plt.plot(t, dT_dis, label='Distance to (dT/dt)_max', color='orange')
plt.axvline(x=67.5, ls='--', label='Detonation limit')
plt.legend(loc=0)
plt.ylabel('$Distance$ [mm]')
plt.xlabel('$Volume\ fraction\ of\ N2\ in\ the\ mixture $ [%]')
plt.savefig('Distance to dTdt.png')
plt.show()


#Reaction zone width
RZW=[0.05477, 0.06323, 0.07539, 0.08377, 0.094554, 0.1079, 0.1251, 0.1482, 0.1811, 0.3047, 0.3602, 0.4365]

plt.clf()
plt.plot(t, RZW, label='Reaction zone width', color='orange')
plt.axvline(x=67.5, ls='--', label='Detonation limit')
plt.legend(loc=0)
plt.ylabel('$Reaction\ zone\ width$ [mm]')
plt.xlabel('$Volume\ fraction\ of\ N2\ in\ the\ mixture $ [%]')
plt.savefig('Reaction zone width.png')
plt.show()
